...
vector<vector<RWLock> > locks_letras;
RWLock lock_tablero;
...
int main(int argc, const char* argv[]) {
    ...
    locks_letras = vector<vector<RWLock> >(alto, vector<RWLock>(ancho, RWLock()));
    ...
    while (true) {
        if ((socketfd_cliente = accept(socket_servidor, (struct sockaddr*) &remoto, (socklen_t*) &socket_size)) == -1) {
            ...
            if (pthread_create(&thread, NULL, atendedor_de_jugador, (void *)&socketfd_cliente)) {
                cerr << "Error al atender cliente" << endl;
                goto error;
            }
...
void *atendedor_de_jugador(void *socket_fd_in) {
    int socket_fd = *((int *)socket_fd_in);
    ...
    while (true) {
        ...
        if (comando == MSG_LETRA) {
            ...
            locks_letras[ficha.fila][ficha.columna].wlock();
            ...
            if (es_ficha_valida_en_palabra(ficha, palabra_actual)) {
                ...
                locks_letras[ficha.fila][ficha.columna].wunlock();
...
void quitar_letras(list<Casillero>& palabra_actual) {
    for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
        locks_letras[casillero->fila][casillero->columna].wlock();
        tablero_letras[casillero->fila][casillero->columna] = VACIO;
        locks_letras[casillero->fila][casillero->columna].wunlock();
    }
